...
| Isolated clean-Windows smoke | Windows Sandbox or physical target run using `docs/testing/target-clean-machine-smoke-operator-packet.md`, recorded in `docs/testing/target-clean-machine-smoke-result-log.md` | `e829507` package prepared; host Sandbox app crashes before bootstrap | DevOps + host owner | Blocked; required before tester handoff unless owner accepts residual risk |
...
| R2 | Isolated clean-Windows release smoke cannot start because Sandbox app 0.8.107.0 is missing `WinRT.Runtime 2.2.0.0`. | High | Host blocker recorded; smoke not executed | DevOps + host owner | Restart the host and retry the immutable package; repair/reset Sandbox if the crash persists. |
...
| --- | --- | --- | --- |
| P0 | Restart host, retry, and record the prepared Windows Sandbox package before tester invite | DevOps + host owner | Operator result block labeled `Windows Sandbox surrogate`, or an owner acceptance of the recorded blocker |
